...
class ValidationErrorFormatter:
    ...
    def _format_html(cls, report: Dict) -> str:
        ...
        if report['valid']:
            ...
            html.append(f'<h4>❌ Validazione Fallita</h4>')
            html.append(f'<p>Trovati {report["error_count"]} errori che devono essere corretti.</p>')
        ...
        if report['errors']:
            ...
            for error in report['errors']:
                html.append('<li class="alert alert-danger" style="padding: 10px; margin: 5px 0;">')

                # Icona e messaggio principale
                html.append(f'<strong>🚫 {cls._get_friendly_message(error)}</strong><br>')

                # Campo interessato
                if error.get('field'):
                    field_name = cls.FIELD_NAMES.get(error['field'], error['field'])
                    html.append(f'<span class="text-muted">Campo: {field_name}</span><br>')

                # Suggerimento
                if error.get('suggestion'):
                    html.append(f'<span class="text-info">💡 {error["suggestion"]}</span><br>')

                # Dettagli tecnici (collassabili)
                if error.get('details'):
                    html.append('<details style="margin-top: 5px;">')
                    html.append('<summary style="cursor: pointer; color: #666;">Dettagli tecnici</summary>')
                    html.append(f'<code style="font-size: 0.85em;">{error["details"][:200]}</code>')
                    html.append('</details>')
        ...
        if report['warnings']:
            ...
            for warning in report['warnings']:
                html.append('<li class="alert alert-warning" style="padding: 8px; margin: 3px 0;">')
                html.append(f'<strong>⚠️ {cls._get_friendly_message(warning)}</strong>')

                if warning.get('suggestion'):
                    html.append(f' - <span class="text-muted">{warning["suggestion"]}</span>')
    ...
    def _format_text(cls, report: Dict) -> str:
        ...
        if report['valid']:
            ...
            lines.append("=" * 50)
            lines.append(f"Errori: {report['error_count']} | Avvisi: {report['warning_count']}")
