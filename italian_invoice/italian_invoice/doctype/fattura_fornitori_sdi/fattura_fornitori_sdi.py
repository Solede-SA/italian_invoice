...
class FatturaFornitoriSDI(Document):
    def validate(self):
        ...
        if not frappe.conf.get("developer_mode"):
            if self.is_new() and not self.get("via_webhook"):
                frappe.throw(
                    "In production mode, new records can only be created via webhook"
                )
            ...
            if not self.is_new() and (
                ...
                frappe.throw(
                    "In production mode, invoice data cannot be modified directly"
                )
    ...
    def _get_tipo_documento(self):
        ...
        try:
            dati = (
                json.loads(self.dati_fattura)
                if isinstance(self.dati_fattura, str)
                else self.dati_fattura
            )
