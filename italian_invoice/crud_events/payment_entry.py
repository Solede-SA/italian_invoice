...
def handle_rounding(doc, method):
	...
	if total_rounding > 0 and total_rounding <= threshold:
		...
		doc._rounding_details = {
			...
			"reference_invoice": (doc.references[0].reference_name if doc.references else None),
		}
...
def validate_rounding_on_submit(doc, method):
	...
	rounding_deductions = (
		[d for d in doc.deductions if d.account == rounding_account] if rounding_account else []
	)
